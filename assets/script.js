...
async function deleteApplication(id) {
    ...
    container.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-secondary);">⏳ Loading Google Sheets Data...</div>';

    const token = getLocalStorage('chaterlabAuthToken', '');

    try {
        const response = await apiFetch(`${API_BASE_URL}/api/director-stats`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        container.innerHTML = `
            <div class="kpi-grid" style="margin-bottom: 30px; border-bottom: 1px solid var(--border-color); padding-bottom: 20px;">
                
                <div class="kpi-card" style="border-left: 4px solid #3b82f6;">
                    <p class="kpi-card-title">👨‍💻 Misha (Old)</p>
                    <div style="font-size: 24px; font-weight: 700; color: var(--text-primary); margin: 5px 0;">${data.misha.leads} <span style="font-size:14px; font-weight:400; color:var(--text-secondary);">leads</span></div>
                    <div style="font-size: 14px; color: var(--text-secondary);">Spend: <b>$${data.misha.spend}</b></div>
                    <div style="font-size: 14px; color: #3b82f6; margin-top:5px;">CPL: <b>$${data.misha.cpl}</b></div>
                </div>

                <div class="kpi-card" style="border-left: 4px solid #8b5cf6;">
                    <p class="kpi-card-title">👩‍💻 Alina (FB)</p>
                    <div style="font-size: 24px; font-weight: 700; color: var(--text-primary); margin: 5px 0;">${data.alina.leads} <span style="font-size:14px; font-weight:400; color:var(--text-secondary);">leads</span></div>
                    <div style="font-size: 14px; color: var(--text-secondary);">Spend: <b>$${data.alina.spend}</b></div>
                ...
                <div class="kpi-card" style="border-left: 4px solid #10b981; background: var(--background-card);">
                    <p class="kpi-card-title">💰 TOTAL (Month)</p>
                    ...
                    <div style="display:flex; justify-content:space-between; margin-top:10px; font-size:13px; color:var(--text-primary);">
                        <span>Leads: <b>${data.total.leads}</b></span>
                    ...
                    <div style="border-top:1px solid var(--border-color); margin-top:8px; padding-top:8px; font-size:13px; color:var(--text-primary);">
                        Interns: <b>${data.total.interns}</b> (CVR: ${data.total.conversion})
                    </div>
                </div>
        ...
        container.innerHTML = `<div style="color: var(--error-color); padding: 10px;">Analytics Error: ${e.message}</div>`;
    }
}